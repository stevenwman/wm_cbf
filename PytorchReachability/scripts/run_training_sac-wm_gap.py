...
def get_args():
    ...
    configs = yml.load(
        (pathlib.Path(sys.argv[0]).parent / "../configs_gap_sac.yaml").read_text()
    )
...
state_dict = {k[14:]:v for k,v in checkpoint['agent_state_dict'].items() if '_wm._orig_mod' in k}
...
actor_activation = getattr(torch.nn, config.actor_activation)
critic_activation = getattr(torch.nn, config.critic_activation)
...
log_alpha = torch.tensor([np.log(0.2)], requires_grad=True, device=config.device)
...
policy = SACPolicy(
    ...
    alpha = 0.1,
    # alpha = (config.target_entropy, log_alpha, alpha_optim) if config.auto_alpha else config.alpha,
...
def get_latent(wm, thetas, imgs):
    ...
    lz = torch.tanh(wm.heads["margin_gp"](feat))
