...
# the one that is used in arXiv:2203.10142
...
from PyHJ.data import Batch, ReplayBuffer, to_torch_as, to_numpy
from PyHJ.utils import MultipleLRSchedulers
from abc import ABC, abstractmethod
from typing import Any, Callable, Dict, List, Optional, Tuple, Union


class BasePolicy_Time_Discounted_Reach_Avoid(ABC, nn.Module):
    def __init__(
        self,
    ...
    def compute_episodic_return(
        ...
        if v_s_ is None:
            ...
            v_s_ = v_s_ * BasePolicy_Time_Discounted_Reach_Avoid.value_mask(buffer, indice)
    ...
    def compute_nstep_return(self,
        ...
        target_q = target_q * BasePolicy_Time_Discounted_Reach_Avoid.value_mask(buffer, terminal).reshape(-1, 1)
        end_flag = buffer.done.copy()
        end_flag[buffer.unfinished_index()] = True
        target_q = _nstep_return_reach_avoid_Bellman_equation(const, end_flag, rew, target_q, indices, gamma)
...
# @njit
def _nstep_return_reach_avoid_Bellman_equation(
    const: np.ndarray,
    end_flag: np.ndarray,
    rew: np.ndarray,
    target_q: np.ndarray,
    ...
    # NOTE! The following two options are for terminal value, i.e., the state with done variable = True:
    # We find that the second option is better for the reach-avoid game, where the terminal value is set to be the target_q, without any intervention. 
    # The intuition is that we leverage the representation power of deep NNs and let them learn the optimal terminal value for the state with Done variable = True.
    # Due to the contraction mapping property of our Bellman operator, the terminal value will converge to the optimal value, when the deep NNs are trained properly.
    
    # Option 1 for terminal value, i.e., the state with done variable = True:
    # d = end_flag[indices]
    # terminal_value = (1-d).reshape(-1,1)*target_q
    
    # Option 2 for terminal value, i.e., the state with done variable = True:
    terminal_value = target_q
    
    # The following line is the Bellman equation used in arXiv:2203.10142
    # V(x) = min(c(x), max( r(x), gamma *  max_u min_d V(f(x,u,d)) ))
    target_q = np.minimum(
        const[indices].reshape(bsz,1), np.maximum(
            rew[indices].reshape(bsz,1), 
            gamma*terminal_value
            )
        )
